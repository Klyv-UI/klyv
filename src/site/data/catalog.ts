...
export const catalog: CatalogEntry[] = [
  ...
  { name: 'JustifiedText', slug: 'justified-text', group: 'Foundations', section: 'Primitives', blurb: 'Justifies a paragraph with Knuth–Plass line breaking over measured widths and Liang hyphenation, re-laid out on resize or font change.' },
  ...
  { name: 'MessageFormat', slug: 'message-format', group: 'Foundations', section: 'Utilities', blurb: 'An ICU MessageFormat parser and formatter — plural, ordinal, select, offset — with plural forms from Intl.PluralRules and errors at their position.' },
  { name: 'TabCoordinator', slug: 'tab-coordinator', group: 'Foundations', section: 'Utilities', blurb: 'Elects one leader tab with Web Locks and passes messages over BroadcastChannel, with a panel showing this tab’s role, its peers and the log.' },
  { name: 'UrlState', slug: 'url-state', group: 'Foundations', section: 'Utilities', blurb: 'useUrlState keeps typed state in the query string, with push or replace, debounced writes, Back and Forward, and several keys in one history entry.' },
  // Layout / Structure
  ...
  { name: 'DockLayout', slug: 'dock-layout', group: 'Layout', section: 'Structure', blurb: 'IDE-style docking of tab groups in resizable splits: drag a tab onto a centre or edge, every action also from the keyboard, as a serialisable tree.' },
  // Layout / Disclosure
  ...
  { name: 'FuzzyFinder', slug: 'fuzzy-finder', group: 'Navigation', section: 'Menus & search', blurb: 'fzf-style fuzzy scoring that picks the best character placement and highlights it, responsive across ten thousand items.' },
  { name: 'FullTextSearch', slug: 'full-text-search', group: 'Navigation', section: 'Menus & search', blurb: 'A client-side inverted index with Porter stemming, BM25 ranking, title boost, prefix matching and marked snippets, updated in place.' },
  { name: 'QueryBar', slug: 'query-bar', group: 'Navigation', section: 'Menus & search', blurb: 'A filter-syntax search field parsed into a tree, with recognised filters as inline chips, schema completion for keys and values, and errors by column.' },
  // Actions / Buttons
  ...
  { name: 'PairwiseRanker', slug: 'pairwise-ranker', group: 'Forms & Inputs', section: 'Choice', blurb: 'Ranks a list by asking which of two matters more, using binary insertion to ask the fewest questions, with ties, skip, undo and arrow keys.' },
  ...
  { name: 'GeoCoordinateInput', slug: 'geo-coordinate-input', group: 'Forms & Inputs', section: 'Numeric & range', blurb: 'Latitude and longitude in any common notation, flagging out-of-range values, read back in words, with distance and bearing to a reference point.' },
  ...
  { name: 'CronEditor', slug: 'cron-editor', group: 'Forms & Inputs', section: 'Date & time', blurb: 'Cron edited field by field with errors in place, described in words, with the next runs in any time zone and daylight-saving shifts flagged.' },
  ...
  { name: 'PathEditor', slug: 'path-editor', group: 'Forms & Inputs', section: 'Rich input', blurb: 'A pen tool for SVG paths: corner and smooth anchors, curve splitting that keeps the shape, keyboard nudging, and normalised path data in and out.' },
  { name: 'ReadabilityMeter', slug: 'readability-meter', group: 'Forms & Inputs', section: 'Rich input', blurb: 'A writing field that grades itself live: Flesch scores and inline marks for hard sentences, passive voice, adverbs and plainer words.' },
  ...
  { name: 'DraftRecovery', slug: 'draft-recovery', group: 'Forms & Inputs', section: 'Form structure', blurb: 'Throttled IndexedDB autosave that offers a newer draft back with a summary of what changed, schema migration, and a memory fallback.' },
  ...
  { name: 'JsonDiff', slug: 'json-diff', group: 'Data Display', section: 'Records', blurb: 'A structural JSON diff with records matched by id so moves read as moves, a change list of JSON Pointer paths and a side-by-side folding tree.' },
  { name: 'JsonQuery', slug: 'json-query', group: 'Data Display', section: 'Records', blurb: 'A JSONPath evaluator with recursive descent, slices, unions and filters parsed without eval, results by path and marked in the document tree.' },
  { name: 'AnsiOutput', slug: 'ansi-output', group: 'Data Display', section: 'Records', blurb: 'Streamed terminal output with 16, 256 and truecolour styles, progress bars that redraw in place, safe links, and colours that follow the theme.' },
  { name: 'ExifViewer', slug: 'exif-viewer', group: 'Data Display', section: 'Records', blurb: 'Reads a JPEG’s EXIF — camera, exposure, date, GPS in decimal — and strips it byte-for-byte without re-encoding, verified before download.' },
  { name: 'ZipBrowser', slug: 'zip-browser', group: 'Data Display', section: 'Records', blurb: 'Opens a ZIP in the browser from its central directory as a keyboard tree, inflating one entry at a time with CRC-32 checked.' },
  { name: 'ResourceScheduler', slug: 'resource-scheduler', group: 'Data Display', section: 'Records', blurb: 'A booking grid of rooms or people against time: drag to book, move or resize, with snapping, overlap rules, a now line and keyboard use.' },
  { name: 'TournamentBracket', slug: 'tournament-bracket', group: 'Data Display', section: 'Records', blurb: 'Single or double knockout brackets from a seeded list, with byes placed by seeding, winners advanced from scores, and a grand-final reset.' },
  // Data Display / Media
  ...
  { name: 'ImageAdjust', slug: 'image-adjust', group: 'Data Display', section: 'Media', blurb: 'Exposure, contrast, saturation, white balance, levels and per-channel tone curves on real pixels, with a live histogram and a spline that never overshoots.' },
  { name: 'SmartCrop', slug: 'smart-crop', group: 'Data Display', section: 'Media', blurb: 'Crop suggestions per aspect ratio that keep the subject, scored by edges, skin tone and colour on a rule-of-thirds search, then adjustable by hand.' },
  { name: 'DocumentScanner', slug: 'document-scanner', group: 'Data Display', section: 'Media', blurb: 'Finds the page in a photo, lets you correct the corners, and flattens it with a true perspective warp and an optional scan look.' },
  { name: 'PaletteExtractor', slug: 'palette-extractor', group: 'Data Display', section: 'Media', blurb: 'An image’s dominant colours by median cut and k-means, with share, hex, OKLCH and contrast-safe text, copied as CSS variables.' },
  { name: 'ModelViewer', slug: 'model-viewer', group: 'Data Display', section: 'Media', blurb: 'A WebGL viewer for binary and ASCII STL and OBJ with its own parsers, fit-to-view, quaternion arcball rotation, wireframe and bounds.' },
  { name: 'GifRecorder', slug: 'gif-recorder', group: 'Data Display', section: 'Media', blurb: 'Records a canvas or element to an animated GIF with its own encoder: median-cut palettes, dithering, LZW and looping.' },
  // Charts / Plots
  ...
  { name: 'ContourPlot', slug: 'contour-plot', group: 'Charts', section: 'Distribution', blurb: '2D kernel density traced with marching squares into filled bands, each named by the share of points it holds, over an optional scatter.' },
  { name: 'ViolinPlot', slug: 'violin-plot', group: 'Charts', section: 'Distribution', blurb: 'Per-group KDE violins with Silverman bandwidth, median and quartiles inside, an optional box and jittered points, vertical or horizontal.' },
  { name: 'HexbinChart', slug: 'hexbin-chart', group: 'Charts', section: 'Distribution', blurb: 'Fifty thousand points counted into hexagons with a stepped count scale, a live bin-radius slider, a legend and a per-bin tooltip.' },
  ...
  { name: 'FlameGraph', slug: 'flame-graph', group: 'Charts', section: 'Flow & hierarchy', blurb: 'Collapsed stacks folded into a flame or icicle graph with zoom and breadcrumb, search with the matched share, and a differential view against a baseline.' },
  { name: 'FlowDiagram', slug: 'flow-diagram', group: 'Charts', section: 'Flow & hierarchy', blurb: 'Flows written as text or data and laid out in ranks automatically, with few crossings, loops allowed, pan, zoom and fit, and keyboard walking along edges.' },
  { name: 'Dendrogram', slug: 'dendrogram', group: 'Charts', section: 'Flow & hierarchy', blurb: 'Hierarchical clustering drawn as a tree, with a cut line you drag to choose the groups and the data matrix reordered beside it to match.' },
  { name: 'MindMap', slug: 'mind-map', group: 'Charts', section: 'Flow & hierarchy', blurb: 'A two-sided mind map built from the keyboard: Tab adds a child, Enter a sibling, arrows move spatially, and it exports as an outline.' },
  ...
  { name: 'TimeSeriesExplorer', slug: 'time-series-explorer', group: 'Charts', section: 'Time & activity', blurb: '100k+ sample series drawn through LTTB at pixel width, with an overview brush, zoom, pan and reset, and a tooltip that reads the raw nearest sample.' },
  { name: 'ForecastChart', slug: 'forecast-chart', group: 'Charts', section: 'Time & activity', blurb: 'A Holt–Winters forecast with fitted weights, a widening prediction band, a history and forecast split, and MAPE on the fit and a held-out horizon.' },
  { name: 'ControlChart', slug: 'control-chart', group: 'Charts', section: 'Time & activity', blurb: 'An XmR process control chart: moving-range sigma zones, Western Electric run rules ringed and explained, and limits recalculated per phase.' },
  { name: 'KaplanMeierChart', slug: 'kaplan-meier-chart', group: 'Charts', section: 'Time & activity', blurb: 'Kaplan–Meier survival curves with censor ticks, Greenwood bands, median markers, an at-risk table and a log-rank p-value across groups.' },
  { name: 'TraceWaterfall', slug: 'trace-waterfall', group: 'Charts', section: 'Time & activity', blurb: 'A distributed trace as a span tree on a shared time axis, with self time, the critical path marked, collapsible rows and a span details panel.' },
  // Charts / Chart parts
  ...
  { name: 'SlaTimer', slug: 'sla-timer', group: 'Feedback', section: 'System state', blurb: 'A countdown to a service target that counts only business hours in a time zone, handling holidays, pauses and clock changes, and saying why it is paused.' },
  ...
  { name: 'PrismLight', slug: 'prism-light', group: 'Motion & Effects', section: 'Surfaces & light', blurb: 'A beam through glass you can turn: Snell’s law per wavelength with Cauchy dispersion, so white light fans into a real spectrum, and past the critical angle the beam reflects instead.' },
  { name: 'SnowDrift', slug: 'snow-drift', group: 'Motion & Effects', section: 'Surfaces & light', blurb: 'Snow that lands on the page’s own elements: every flake settles on the heightfield above the real headings and cards, piles up, avalanches when the slope is too steep, and blows off when you sweep the pointer.' },
  ...
  { name: 'MeasureTool', slug: 'measure-tool', group: 'Interaction', section: 'Touch & drag', blurb: 'Calibrate from a known length, then measure distances, areas and angles on an image in real units, with editable points and keyboard placement.' },
  { name: 'TransformBox', slug: 'transform-box', group: 'Interaction', section: 'Touch & drag', blurb: 'A move, resize and rotate frame that keeps the opposite corner fixed on rotated boxes, with Shift for proportions and Alt to resize from the centre.' },
  { name: 'SnapGuides', slug: 'snap-guides', group: 'Interaction', section: 'Touch & drag', blurb: 'Boxes that snap to each other’s edges and centres while dragging, showing distances and equal-spacing marks; hold Alt to place freely.' },
  { name: 'StrokeGestures', slug: 'stroke-gestures', group: 'Interaction', section: 'Touch & drag', blurb: 'Draw a gesture and it is recognised, scored and mapped to an action; it learns new gestures from one drawing, and each gesture is also a button.' },
  ...
  { name: 'RankedChoiceResults', slug: 'ranked-choice-results', group: 'Interaction', section: 'Presence & collaboration', blurb: 'Ranked-choice results round by round: eliminations, where ballots went, exhausted ballots, tie-breaks, and the full count as a table.' },
  { name: 'CommentAnchors', slug: 'comment-anchors', group: 'Interaction', section: 'Presence & collaboration', blurb: 'Comments pinned to a quote, not an offset: they re-anchor by exact then fuzzy match with a confidence score, and orphans are listed openly.' },
  { name: 'CollaborativeText', slug: 'collaborative-text', group: 'Interaction', section: 'Presence & collaboration', blurb: 'A textarea bound to an RGA sequence CRDT, so peers can edit offline or out of order and still converge, optionally across tabs.' },
  { name: 'PeerLink', slug: 'peer-link', group: 'Interaction', section: 'Presence & collaboration', blurb: 'Browser-to-browser text and files over WebRTC, with copy-paste or BroadcastChannel signalling, back-pressure and SHA-256 checks.' },
  { name: 'SlideDeck', slug: 'slide-deck', group: 'Interaction', section: 'Presence & collaboration', blurb: 'A presentation with keyboard, a URL per slide, fullscreen, an overview grid and a presenter window synced over BroadcastChannel.' },
  ...
  { name: 'ThreeWayMerge', slug: 'three-way-merge', group: 'Interaction', section: 'Trust & workflow', blurb: 'A diff3 merge of base, ours and theirs: clean changes merge themselves, conflicts get ours, theirs, both or edit, and git markers otherwise.' },
  { name: 'Redactor', slug: 'redactor', group: 'Interaction', section: 'Trust & workflow', blurb: 'Finds emails, phones, Luhn-valid cards, mod-97 IBANs, keys, JWTs and IPs, and redacts only what you approve, before and after side by side.' },
  ...
  { name: 'PaperMarbling', slug: 'paper-marbling', group: 'Canvas & Play', section: 'Generative', blurb: 'Ebru marbling done exactly: every drop pushes the ink already on the bath outwards by the closed-form circle map, and a tine drag shifts it along a decaying wake, so no pattern is ever painted — it is displaced.' },
  { name: 'VoxelTerrain', slug: 'voxel-terrain', group: 'Canvas & Play', section: 'Generative', blurb: 'Voxel-space terrain flown from a heightmap generated by diamond–square: back-to-front column casting with a y-buffer, so a ridge line resolves per pixel with no polygons at all.' },
  // Canvas & Play / Audio
  ...
  { name: 'PendulumWave', slug: 'pendulum-wave', group: 'Canvas & Play', section: 'Physics', blurb: 'Fifteen pendulums whose lengths are tuned so the slowest swings sixty times a minute and each next one once more: they fan, snake, split into two waves and return to one line, on a cycle that closes exactly.' },
  // Canvas & Play / Toys
  ...
  { name: 'ChessBoard', slug: 'chess-board', group: 'Canvas & Play', section: 'Toys', blurb: 'Chess with every rule — pins, castling, en passant, promotion, all the draws — plus SAN, FEN and PGN, and full keyboard play with spoken moves.' },
  { name: 'Game2048', slug: 'game-2048', group: 'Canvas & Play', section: 'Toys', blurb: '2048 with tiles that keep their identity, so slides and merges animate; one merge per tile, undo, best score, arrows, WASD and swipe.' },
  { name: 'WordGuess', slug: 'word-guess', group: 'Canvas & Play', section: 'Toys', blurb: 'A five-letter word game with correct duplicate-letter scoring, hard mode, daily or random words, a stateful keyboard and an emoji share grid.' },
  { name: 'Solitaire', slug: 'solitaire', group: 'Canvas & Play', section: 'Toys', blurb: 'Klondike with draw one or three, undo, scoring, a timer, auto-send and finish, drag or full keyboard play, and every card named in full.' },
  { name: 'OrbitSandbox', slug: 'orbit-sandbox', group: 'Canvas & Play', section: 'Toys', blurb: 'Fling a planet and watch it fall into orbit: velocity Verlet n-body gravity with real conic trails, collisions that merge and conserve momentum, and a readout of the energy it is not allowed to lose.' },
  { name: 'Tesseract', slug: 'tesseract', group: 'Canvas & Play', section: 'Toys', blurb: 'A four-dimensional solid turned in two planes at once: rotate in xw and yw, watch the inner cube swell through the outer as it passes, and switch the projection from perspective to stereographic.' },
  // SaaS / Marketing
  ...
  { name: 'ExpenseSplitter', slug: 'expense-splitter', group: 'SaaS', section: 'Billing', blurb: 'Splits costs equally, by shares, exact amounts or percentages in whole cents, then settles with as few transfers as matching finds.' },
  // SaaS / Workspace
  ...
  { name: 'RegexTester', slug: 'regex-tester', group: 'SaaS', section: 'Settings & developer', blurb: 'Live matches with numbered and named groups, the pattern explained token by token, and patterns that could hang flagged and time-boxed in a Worker.' },
  { name: 'CurlConverter', slug: 'curl-converter', group: 'SaaS', section: 'Settings & developer', blurb: 'Splits a pasted curl command the way a shell would, applies curl’s flag rules, and writes fetch, axios and Python code, listing anything unconverted.' },
  { name: 'SemverRange', slug: 'semver-range', group: 'SaaS', section: 'Settings & developer', blurb: 'Tests versions against an npm range with npm’s prerelease rule, showing what the range expands to, which versions match and the highest.' },
  { name: 'ErrorBudget', slug: 'error-budget', group: 'SaaS', section: 'Settings & developer', blurb: 'An SLO error budget with burn-down, burn rate, time to exhaustion and multi-window burn-rate alerts evaluated over the whole series.' },
  // SaaS / Data & views
  ...
  { name: 'CsvImport', slug: 'csv-import', group: 'SaaS', section: 'Data & views', blurb: 'An RFC 4180 parser read in chunks with progress, sniffing the delimiter, header and column types, and listing bad cells by line before import.' },
  { name: 'SqlBuilder', slug: 'sql-builder', group: 'SaaS', section: 'Data & views', blurb: 'A schema-aware query builder that finds joins along foreign keys, suggests GROUP BY and writes quoted PostgreSQL or MySQL as you go.' },
  { name: 'LogPatterns', slug: 'log-patterns', group: 'SaaS', section: 'Data & views', blurb: 'Groups raw log lines into templates with the Drain parse tree — numbers, IPs and ids masked, counts and variable slots per template, incremental on a live tail.' },
  { name: 'RecordMerge', slug: 'record-merge', group: 'SaaS', section: 'Data & views', blurb: 'Finds duplicate records with blocking, Jaro–Winkler and token-set scoring and union-find clusters, then reviews each merge with per-field survivorship.' },
  // SaaS / Engagement
  ...
  { name: 'SpacedRepetition', slug: 'spaced-repetition', group: 'SaaS', section: 'Engagement', blurb: 'Flashcards on the SM-2 schedule: a due queue, graded reviews with next-interval previews, retention stats and a review forecast.' },
  // SaaS / Security
  ...
  { name: 'CspEvaluator', slug: 'csp-evaluator', group: 'SaaS', section: 'Security', blurb: 'Reviews a Content-Security-Policy by severity and shows what each resource type allows after default-src fallback, striking ignored sources.' },
  { name: 'JwtInspector', slug: 'jwt-inspector', group: 'SaaS', section: 'Security', blurb: 'Decodes and explains a JWT with live expiry countdowns, verifies HS, RS, PS and ES signatures with Web Crypto, and flags alg none.' },
]
