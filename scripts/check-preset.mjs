// Compiles `klyvui/preset.css` the way an installed consumer does, and fails
...
/** What `@import 'klyvui/preset.css'` resolves to from a consumer's stylesheet. */
function resolveStylesheet(id, base) {
  if (id === 'tailwindcss') return join(ROOT, 'node_modules', 'tailwindcss', 'index.css')
  if (id === 'klyvui' || id.startsWith('klyvui/')) {
    const key = id === 'klyvui' ? '.' : `./${id.slice('klyvui/'.length)}`
...
const consumer = ["@import 'tailwindcss';", "@import 'klyvui/preset.css';", ''].join('\n')
